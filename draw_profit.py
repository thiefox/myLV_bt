...
def draw_kline_and_profit(b_year : int, e_year : int, interval : kline_interval, func : callable):
    if func is None :
        return draw_klines(b_year, e_year, interval)
    if e_year - b_year == 1:
        UNIT = 'M'
    else :
        UNIT = 'Y'
    ...
    if klines:
        ...
        dates = np.array(dates)
        closed_prices = [float(kline[4]) for kline in klines]
        profits = func(b_year, e_year, interval)
